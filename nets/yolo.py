...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, backbone='cspdarknet', pretrained=False, input_shape=[640, 640]):
        ...
        if backbone == "cspdarknet":
            ...
            #   20,20,1024
            #---------------------------------------------------#
            print("gyf:base_channels={}".format(base_channels))#gyf: 32
            print("gyf:base_depth={}".format(base_depth)) #gyf: 1
            print("gyf:pretrained={}".format(pretrained)) #gyf: False
    ...
    def forward(self, x):
        #  backbone
        print("gyf:x.shape input backbone={}".format(x.shape))
        feat1, feat2, feat3 = self.backbone(x)
        # print("gyf:feat1.shape={}".format(feat1.shape))
        # print("gyf:feat2.shape={}".format(feat2.shape))
        # print("gyf:feat3.shape={}".format(feat3.shape))
        ...
        #print("gyf:P5.shape={}".format(P5.shape))
        # 20, 20, 512 -> 40, 40, 512
        P5_upsample = self.upsample(P5)
        #print("gyf:P5_upsample.shape={}".format(P5_upsample.shape))
        ...
        # print("gyf:P3.shape={}".format(P3.shape))#torch.Size([2, 128, 80, 80])
        # print("gyf:P4.shape={}".format(P4.shape))#torch.Size([2, 256, 40, 40])
        # print("gyf:P5.shape={}".format(P5.shape))#torch.Size([2, 512, 20, 20])
        # print("gyf:out2.shape={}".format(out2.shape))#torch.Size([2, 18, 80, 80])
        # print("gyf:out1.shape={}".format(out1.shape))#torch.Size([2, 18, 40, 40])
        # print("gyf:out0.shape={}".format(out0.shape))#torch.Size([2, 18, 20, 20])
