...
class YOLOLoss(nn.Module):
    ...
    def forward(self, l, input, targets=None, y_true=None):
        ...
        #-------------------------------------------------#
        #print("gyf:self.anchors={}".format(self.anchors)) #gyf:所有的９个anchors
        scaled_anchors  = [(a_w / stride_w, a_h / stride_h) for a_w, a_h in self.anchors]
        #print("gyf:scaled_anchors={}".format(scaled_anchors))
        #-----------------------------------------------#
    ...
    def get_pred_boxes(self, l, x, y, h, w, targets, scaled_anchors, in_h, in_w):
        ...
        print("gyf:grid_x={}".format(grid_x))
        print("gyf:grid_y={}".format(grid_y))
        print("gyf:grid_x.shape={}".format(grid_x.shape))
        print("gyf:grid_y.shape={}".format(grid_y.shape))

        # 生成先验框的宽高
        print("gyf:self.anchors_mask[l]={}".format(self.anchors_mask[l]))
        print("gyf:np.array(scaled_anchors)={}".format(np.array(scaled_anchors)))
        print("gyf:np.array(scaled_anchors)[]={}".format(np.array(np.array(scaled_anchors)[self.anchors_mask[l]])))
        scaled_anchors_l = np.array(scaled_anchors)[self.anchors_mask[l]]
        anchor_w = torch.Tensor(scaled_anchors_l).index_select(1, torch.LongTensor([0])).type_as(x)
        anchor_h = torch.Tensor(scaled_anchors_l).index_select(1, torch.LongTensor([1])).type_as(x)
        print("gyf:anchor_w={}".format(anchor_w))
        print("gyf:anchor_h={}".format(anchor_h))
        
        anchor_w = anchor_w.repeat(bs, 1).repeat(1, 1, in_h * in_w).view(w.shape)
        anchor_h = anchor_h.repeat(bs, 1).repeat(1, 1, in_h * in_w).view(h.shape)
        print("gyf:anchor_w={}".format(anchor_w))
        print("gyf:anchor_h={}".format(anchor_h))
        print("gyf:anchor_w.shape={}".format(anchor_w.shape))
        print("gyf:anchor_h.shape={}".format(anchor_h.shape))
        #-------------------------------------------------------#
        #   计算调整后的先验框中心与宽高
        #-------------------------------------------------------#
        ...
        # print("gyf:x={}".format(x))
        # print("gyf:y={}".format(y))
        # print("gyf:w={}".format(w))
        # print("gyf:h={}".format(h))
        # print("gyf:pred_boxes_x={}".format(pred_boxes_x))
        # print("gyf:pred_boxes_y={}".format(pred_boxes_y))
        # print("gyf:pred_boxes_w={}".format(pred_boxes_w))
        # print("gyf:pred_boxes_h={}".format(pred_boxes_h))
