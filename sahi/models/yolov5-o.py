...
class Yolov5DetectionModel(DetectionModel):
    ...
    def _create_object_prediction_list_from_original_predictions(
        ...
        shift_amount_list = fix_shift_amount_list(shift_amount_list)#gyf:仅增加了一个维度，表示图片个数，这里只有一张图，因此，shift_amount_list长度为１
        full_shape_list = fix_full_shape_list(full_shape_list)
        #print("gyf:shift_amount_list after fix={}".format(shift_amount_list))
        # handle all predictions
        object_prediction_list_per_image = []
        print("gyf:type of original_predictions.xyxy={}".format(type(original_predictions.xyxy)))#gyf:list
