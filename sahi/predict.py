...
from utils.utils import cvtColor,resize_image #added by gyf
...
def get_prediction(
    ...
    # read image as pil
    #print("gyf:image.shape before perform_inference={}".format(image.shape))
    #print("gyf:image input perform_inference={}".format(image)) #gyf:image is ndarray
    #image_as_pil = read_image_as_pil(image)
    print("gyf:type of image={}".format(type(image)))
    # get prediction
    time_start = time.time()
    #print("gyf:image.shape input perform_inference={}".format(np.ascontiguousarray(image_as_pil).shape))#(256, 256, 3)
    #detection_model.perform_inference(np.ascontiguousarray(image_data))
    #detection_model.perform_inference(image_as_pil)
    detection_model.perform_inference(image) #gyf:image is ndarray
    ...
    # print("gyf:shift_amount={}".format(shift_amount))#gyf:#切片相对于原图的偏移位置，即左上角的x,y坐标
    # print("gyf:full_shape={}".format(full_shape))#gyf:原大图尺寸，例如[1280, 2048]
    detection_model.convert_original_predictions(
        shift_amount=shift_amount,
        full_shape=full_shape,
    )
    object_prediction_list: List[ObjectPrediction] = detection_model.object_prediction_list##gyf:为一张原图或者一张切片图的所有目标
    for r in range(len(object_prediction_list)):
        print("gyf:r={},bbox={}".format(r,object_prediction_list[r].bbox))
    # postprocess matching predictions
    print("gyf:postprocess={}".format(postprocess))#None
...
def get_sliced_prediction(
    ...
    slice_image_result = slice_image(
        #image=image,#annotated by gyf
        image=image_roi, #added by gyf
    ...
    # for slice_index in range(len(slice_image_result)):
    #     print("gyf:slice_[0].shape={}".format(slice_image_result.images[slice_index].shape))#gyf: (256, 256, 3)
    ...
    for group_ind in range(num_group):#gyf:一张原图有n个切片图，由于目前仅支持一组只能一张切片，所以，一张原图有n组,每组一张切片图
        ...
        # perform batch prediction#==================================>for every slice:gyf
        prediction_result = get_prediction(
            image=image_list[0], #gyf:ndarry of a slice
        ...
        print("gyf:len(prediction_result.object_prediction_list)={}".format(len(prediction_result.object_prediction_list)))
        for object_prediction in prediction_result.object_prediction_list:#gyf:object_prediction为ObjectPrediction对象
            if object_prediction:  # if not empty
                object_prediction_list.append(object_prediction.get_shifted_object_prediction())
        print("gyf:object_prediction_list after shifted={}".format(object_prediction_list))
        # merge matching predictions during sliced prediction
        print("gyf:merge_buffer_length={}".format(merge_buffer_length))#None
        if merge_buffer_length is not None and len(object_prediction_list) > merge_buffer_length:
            object_prediction_list = postprocess(object_prediction_list)

    # perform standard prediction
    perform_standard_pred = False #added by gyf
    ...
    if len(object_prediction_list) > 1:
        print("gyf:len object_prediction_list>1")
        object_prediction_list = postprocess(object_prediction_list)
    ...
    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR) #added by gyf
    return PredictionResult(
        image=image, object_prediction_list=object_prediction_list, durations_in_seconds=durations_in_seconds
    )
...
def predict(
    ...
    **kwargs,
    ...
    for ind, image_path in enumerate(
        ...
        image_as_pil = read_image_as_pil(image_path)
        print("gyf:image_as_pil.size={}".format(image_as_pil.size))

        # perform prediction
        if not no_sliced_prediction:
            # get sliced prediction
            prediction_result = get_sliced_prediction(
                #image=image_as_pil, #annotated by gyf
                image=image_path,
                detection_model=detection_model,
                slice_height=image_as_pil.size[1],
                slice_width=128,
                overlap_height_ratio=overlap_height_ratio,
                overlap_width_ratio=overlap_width_ratio,
