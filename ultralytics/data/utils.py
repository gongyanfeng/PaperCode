...
def check_det_dataset(dataset, autodownload=True):
    ...
    print("gyf:data={}".format(data)) #/home/gyf/e/yolov8-sahi/yolov8/ultralytics/cfg/datasets/coco128.yaml

    # Download (optional)
    extract_dir = ''
    if isinstance(data, (str, Path)) and (zipfile.is_zipfile(data) or is_tarfile(data)): #gyf: no
        new_dir = safe_download(data, dir=DATASETS_DIR, unzip=True, delete=False)
        data = find_dataset_yaml(DATASETS_DIR / new_dir)
        extract_dir, autodownload = data.parent, False

    # Read YAML (optional)
    if isinstance(data, (str, Path)): #gyf:yes
        print("gyf:data is str")
        data = yaml_load(data, append_filename=True)  # dictionary
        print("gyf:data 2={}".format(data))
    ...
    print("gyf:path={}".format(path))#"../datasets/coco128"

    if not path.is_absolute():
        path = (DATASETS_DIR / path).resolve()
        print("gyf:path 2={}".format(path)) #/home/czh/yolov8/ultralytics/datasets/coco128
    ...
    print("gyf:train={},val={},test={}".format(train, val, test))
    if val:
