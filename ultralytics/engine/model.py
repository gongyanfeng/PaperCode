...
class Model(nn.Module):
    ...
    def __init__(self, model: Union[str, Path] = 'yolov8n.pt', task=None) -> None:
        ...
        print("gyf:model={}".format(model)) #model=yolov8n.yaml
        # Check if Ultralytics HUB model from https://hub.ultralytics.com
        if self.is_hub_model(model):
            print("gyf:model is hub_model") #no
        ...
        # Check if Triton Server model
            ...
            print("gyf:model is triton_model") #no
            self.model = model
            self.task = task
            return
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=True):
        ...
        """
        #print("gyf:cfg={}".format(cfg))#yolov8n.yaml
        cfg_dict = yaml_model_load(cfg)
        #print("gyf:cfg_dict={}".format(cfg_dict))
        self.cfg = cfg
        self.task = task or guess_model_task(cfg_dict)
        print("gyf:task={}".format(self.task))#detect
        self.model = (model or self._smart_load('model'))(cfg_dict, verbose=verbose and RANK == -1)  # build model
        print("gyf:model={}".format(self.model))#DetectionModel
    ...
    def train(self, trainer=None, **kwargs):
        ...
        print("gyf:args={}".format(args))
        if args.get('resume'):
            args['resume'] = self.ckpt_path

        self.trainer = (trainer or self._smart_load('trainer'))(overrides=args, _callbacks=self.callbacks) #yolo.detect.DetectionTrainer
